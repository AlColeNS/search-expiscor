...
/**
 * Implements the MySQL RDBMS interfaces for table
...
{
    private final int VENDOR_CHAR_THRESHOLD = 5;
    private final int VENDOR_VARCHAR_THRESHOLD = 65535;
    private final String VENDOR_STORAGE_ENGINE = "InnoDB";
     ...
     */
    public MySQLTable(SQLConnection aConnection)
    {
        super(aConnection);
    ...
    private String columnElement(DataField aField)
    {
        ...
        {
            case Boolean:
                sqlElement = String.format("%s boolean", fieldName);
                break;
            case Integer:
                sqlElement = String.format("%s int(10)", fieldName);
                break;
            case Long:
                sqlElement = String.format("%s bigint", fieldName);
                break;
            case Float:
            case Double:
                sqlElement = String.format("%s double(10,4)", fieldName);
                break;
            case Text:
                if (storageSize < VENDOR_CHAR_THRESHOLD)
                    ...
                    sqlElement = String.format("%s longtext binary", fieldName);
                break;
            case Date:
                sqlElement = String.format("%s date", fieldName);
                break;
            case Time:
                sqlElement = String.format("%s time", fieldName);
                break;
            case DateTime:
                sqlElement = String.format("%s datetime", fieldName);
                break;
            default:
                return fieldName;
        }

        if (aField.isFeatureTrue(Field.FEATURE_IS_REQUIRED))
            sqlElement += " not null";
        ...
        throws NSException
    {
        ...
        for (DataField pField : aBag.getFields())
        {
            if (isCommaNeeded)
                ...
                isCommaNeeded = true;
            if (pField.isFeatureAssigned(Field.FEATURE_FUNCTION_NAME))
    ...
    private String buildWhereClause(DataBag aBag, DSCriteria aCriteria, int anOffset, int aLimit)
        throws NSException
    {
        ...
        if (aLimit >= 0)
        {
            if (sqlBuilder.length() == 0)
                sqlBuilder.append(String.format(" WHERE LIMIT %d,%d", anOffset, aLimit));
            else
                sqlBuilder.append(String.format(" LIMIT %d,%d", anOffset, aLimit));
        }
        ...
        throws NSException
    {
        ...
        if ((aBag == null) || (aBag.count() == 0))
            throw new NSException("Empty data bag.");
        ...
        throws NSException
    {
        ...
        for (DataField pField : aBag.getFields())
        {
            if (SQL.isSequenceExplicit(pField))
        ...
        throws NSException
    {
        ...
        for (DataField pField : aBag.getFields())
        {
            ...
            if (StringUtils.isEmpty(fieldValue))
            {
                ...
                else if (SQL.isSequenceImplicit(pField))
                    isCommaNeeded = false;
                else
        ...
        throws NSException
    {
        ...
        if (aCriteria != null)
            sqlStatement += buildWhereClause(countBag, aCriteria, SQL.CRITERIA_NO_OFFSET, SQL.CRITERIA_NO_LIMITS);
     ...
     */
    @Override
    public DataTable select(DataBag aBag) throws NSException
    {
     ...
     */
    @Override
    public DataTable select(DataBag aBag, DSCriteria aCriteria)
        throws NSException
    {
        Logger appLogger = mAppMgr.getLogger(this, "select");
        ...
        if (aCriteria != null)
            sqlStatement += buildWhereClause(aBag, aCriteria, SQL.CRITERIA_NO_OFFSET, SQL.CRITERIA_NO_LIMITS);
     ...
     * to each operation.
     *
     * @param aBag      Persistent field bag.
     * @param aCriteria Data source criteria.
        ...
        throws NSException
    {
        ...
        if (aCriteria != null)
            sqlStatement += buildWhereClause(aBag, aCriteria, anOffset, aLimit);
     ...
     */
    @Override
    public DataTable select(DataBag aBag, String aWhereClause)
        throws NSException
    {
        Logger appLogger = mAppMgr.getLogger(this, "select");
     ...
     */
    @Override
    public ResultSet select(String aSelectFromWhereStatement)
        throws NSException
    {
        Logger appLogger = mAppMgr.getLogger(this, "select");
        ...
        throws NSException
    {
        ...
        if (aCriteria == null)
        {
            ...
            sqlBuilder.append(buildWhereClause(aBag, aCriteria, SQL.CRITERIA_NO_OFFSET, SQL.CRITERIA_NO_LIMITS));
     ...
     * primary key field in the {@link DataBag} with any fields
     ...
     * primary key field in the {@link DataBag}.
     *
     * @param aBag Data field bag with DB name assigned and a
     *             primary key designated.
     * @throws com.nridge.core.base.std.NSException Catch-all exception for any SQL related issue.
