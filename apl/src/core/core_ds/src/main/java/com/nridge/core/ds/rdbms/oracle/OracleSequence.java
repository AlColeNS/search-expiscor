...
/**
 * Implements the oracle RDBMS interfaces for sequence
 ...
 * @since 1.0
 */
public class OracleSequence extends SQLSequence
{
    /**
     ...
     */
    public OracleSequence(AppMgr anAppMgr, SQLConnection aConnection)
    {
        ...
        throws NSException
    {
        ...
        if (SQL.isSequenceManaged(aField))
        {
            ...
            ResultSet resultSet = sqlTable.select(String.format("SELECT %s.nextval FROM dual", sequenceName));
            try
            {
    ...
    public int currentValue(DataBag aBag, DataField aField) throws NSException
    {
        ...
        if (SQL.isSequenceManaged(aField))
        {
            String sequenceName = schemaName(aBag, aField.getName());
            SQLTable sqlTable = mSQLConnection.newTable();
            ResultSet resultSet = sqlTable.select(String.format("SELECT %s.currval FROM dual", sequenceName));
            try
            {
                if ((resultSet != null) && (resultSet.next()))
                    sequenceValue = resultSet.getInt(1);
    ...
    public String insertValue(DataBag aBag, DataField aField) throws NSException
    {
        ...
        {
            String sequenceName = schemaName(aBag, aField.getName());
            sqlValue = String.format("%s.nextval", sequenceName);
        }
        else
        ...
        throws NSException
    {
        ...
        {
            String sequenceName = schemaName(aBag, aField.getName());
            mSQLConnection.execute(String.format("CREATE SEQUENCE %s START WITH %d INCREMENT BY %d NOCACHE NOCYCLE",
