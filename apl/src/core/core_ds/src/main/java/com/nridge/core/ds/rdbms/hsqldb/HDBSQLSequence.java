...
package com.nridge.core.ds.rdbms.hsqldb;
...
/**
 * Implements the Hypersonic SQL RDBMS interfaces for
 * sequence operations.  Since these methods are specific
 * to a particular RDBMS vendor, an application developer
 ...
 * @since 1.0
 */
public class HDBSQLSequence extends SQLSequence
{
    private final String SEQUENCE_IMPLICIT_VALUE = "DEFAULT";
     ...
     */
    public HDBSQLSequence(AppMgr anAppMgr, SQLConnection aConnection)
    {
        ...
        throws NSException
    {
        ...
        if (SQL.isSequenceManaged(aField))
        {
            ...
            ResultSet resultSet = sqlTable.select(String.format("CALL NEXT VALUE FOR %s", sequenceName));
            try
            {
     ...
     */
    @Override
    public int currentValue(DataBag aBag, DataField aField)
        throws NSException
    {
        int sequenceValue = SQL.VALUE_IS_INVALID;
        ...
        if (SQL.isSequenceManaged(aField))
        {
            String sequenceName = schemaName(aBag, aField.getName());
            SQLTable sqlTable = mSQLConnection.newTable();
            ResultSet resultSet = sqlTable.select(String.format("CALL CURRENT VALUE FOR %s", sequenceName));
            try
            {
                if ((resultSet != null) && (resultSet.next()))
                    sequenceValue = resultSet.getInt(1);
     ...
     */
    @Override
    public String insertValue(DataBag aBag, DataField aField)
        throws NSException
    {
        String sqlValue;
        ...
        if (SQL.isSequenceImplicit(aField))
            sqlValue = SEQUENCE_IMPLICIT_VALUE;
        else if (SQL.isSequenceExplicit(aField))
        {
            String sequenceName = schemaName(aBag, aField.getName());
            sqlValue = String.format("NEXT VALUE FOR %s", sequenceName);
        }
        else
