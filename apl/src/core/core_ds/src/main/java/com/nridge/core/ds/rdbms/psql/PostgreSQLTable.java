...
/**
 * Implements the PostgreSQL RDBMS interfaces for table
...
public class PostgreSQLTable extends SQLTable
{
     ...
     */
    public PostgreSQLTable(SQLConnection aConnection)
    {
        super(aConnection);
...
// http://www.postgresql.org/docs/9.1/static/datatype.html
    ...
    private String columnElement(DataField aField)
    {
        ...
        switch(aField.getType())
        {
            case Boolean:
                ...
                break;
            case Integer:
                sqlElement = String.format("%s integer", fieldName);
                break;
            case Long:
            ...
            case Float:
            case Double:
                sqlElement = String.format("%s numeric(100,10)", fieldName);
                break;
            case Text:
                if (storageSize < VENDOR_CHAR_THRESHOLD)
                    sqlElement = String.format("%s char(%d)", fieldName, storageSize);
                else
                    sqlElement = String.format("%s text", fieldName);
                break;
            case Date:
     ...
     */
    @Override
    public void create(DataBag aBag) throws NSException
    {
        ...
        throws NSException
    {
        ...
        if (anOffset >= 0)
        {
            if (sqlBuilder.length() == 0)
                sqlBuilder.append(" WHERE OFFSET " + anOffset);
            else
                sqlBuilder.append(" OFFSET " + anOffset);
        }
        if (aLimit >= 0)
        {
            if (sqlBuilder.length() == 0)
                sqlBuilder.append(" WHERE LIMIT " + aLimit);
            else
                sqlBuilder.append(" LIMIT " + aLimit);
        }
     ...
     *             primary key designated.
     *
     * @throws com.nridge.core.base.std.NSException Catch-all exception for
     * any SQL related issue.
     */
    @Override
        ...
        throws NSException
    {
        ...
        dsCriteria.add(dataField.getName(), Field.Operator.EQUAL, dataField.getValue());
     ...
     * {@link com.nridge.core.base.ds.DSCriteria}.  The
     * {@link com.nridge.core.base.field.data.DataBag} is used to
