...
import com.nridge.connector.fs.con_fs.core.*;
...
/**
 * The TaskConnectorFS is responsible for driving the file system
...
public class TaskConnectorFS implements Task
{
    private final String mRunName = "cfs";
    private final String mTestName = "cfs";
    ...
    private ExecutorService mExtractExecutor;
    ...
    public DataBag schemaBag()
    {
        ...
        dataTextField = new DataTextField("nsd_parent_id", "NSD Parent Id");
        ...
        dataBag.add(new DataTextField("nsd_url_parent", "NSD URL Parent"));
        ...
        dataTextField.setMultiValueFlag(true);
        dataBag.add(dataTextField);
        dataTextField = new DataTextField("nsd_acl_deny", "NSD ACL Deny");
        dataTextField.setMultiValueFlag(true);
        dataBag.add(dataTextField);
        ...
        dataTextField.setDefaultValue("NSD FS");
        dataBag.add(dataTextField);
        dataTextField = new DataTextField("nsd_con_type", "NSD Connector Type");
        dataTextField.setDefaultValue("File Share Local");
        dataBag.add(dataTextField);
...
// Create Restlet server instance.
    ...
    private boolean isPipelineReady(CrawlQueue aCrawlQueue)
    {
        ...
        boolean isCrawlStartValid = true;
...
// Update RDBMS with new crawl and snapshot information.
...
will control how many threads run concurrently based on how it was configured. */
        ...
        mExtractExecutor = Executors.newFixedThreadPool(extractThreadCount);

        ArrayList<String> crawlStartList = crawlStart.getList();
        for (String crawlStartName : crawlStartList)
        {
            queueThread = new RunExtractFSLocal(mAppMgr, aCrawlQueue, crawlStartName);
            mExtractExecutor.execute(queueThread);
        }
...
will control how many threads run concurrently based on how it was configured. */
        ...
        {
            queueThread = new RunTransformFS(mAppMgr, aCrawlQueue);
...
will control how many threads run concurrently based on how it was configured. */
        ...
        {
            queueThread = new RunPublishFS(mAppMgr, aCrawlQueue);
...
will control how many threads run concurrently based on how it was configured. */
    ...
    private void waitForCompletion(CrawlQueue aCrawlQueue)
    {
        ...
        if (mExtractExecutor != null)
        {
            mExtractExecutor.shutdown();
            while (! mExtractExecutor.isTerminated())
                Sleep.forSeconds(1);
...
// Place a queue item marker into the queue to mark the end of the document extraction process.
    ...
    private void executeReset()
    {
        ...
        if (mExtractExecutor != null)
        {
            ...
            mExtractExecutor.shutdownNow();
            mExtractExecutor = null;
        }
        if (mTransformExecutor != null)
        {
...
// Good time for a garbage collection event.
    ...
    public void run()
    {
        ...
        if (! isExecuteSinglePass())
        {
            ...
            {
                String msgStr = String.format("Waiting %d second(s) before executing first crawl.",
        ...
        while (isAlive())
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (mailManager != null)
                    {
                        mailManager.addMessage(Connector.PHASE_ALL, Connector.PROPERTY_MAIL_NAME,
