...
import com.nridge.connector.ws.con_ws.core.Constants;
...
import com.nridge.ds.solr.SolrConfigXML;
...
/**
 * The TaskSolrConfig reads the NSD schema file and generates
 * a corresponding Solr config file containing field fragments.
 */
@SuppressWarnings("FieldCanBeLocal")
public class TaskSolrConfig implements Task
{
    private final String mRunName = "solr_config";
    private final String mTestName = "solr_config";
    ...
    public void run()
    {
        ...
        {
            appLogger.info("Generating the Solr Configuration file to the log folder.");

            String solrConfigPathFileName = String.format("%s%csolr_config.xml",
                                                          mAppMgr.getString(mAppMgr.APP_PROPERTY_LOG_PATH),
                                                          File.separatorChar);
            try
            {
                Document schemaDocument = new Document("NSD Schema", mSchemaBag);
                SolrConfigXML solrConfigXML = new SolrConfigXML(mAppMgr, schemaDocument);
                solrConfigXML.save(solrConfigPathFileName);
            }
            catch (IOException e)
            {
                String msgStr = String.format("%s: %s", solrConfigPathFileName, e.getMessage());
