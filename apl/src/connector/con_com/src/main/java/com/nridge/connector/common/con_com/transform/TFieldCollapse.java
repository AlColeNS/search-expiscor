...
/**
 * The TFieldCollapse document transformer collapses the
 * fields from source document to the returned destination
 * document based on an external property file.
 *
 * @since 1.0
 * @author Al Cole
 */
public class TFieldCollapse implements TransformInterface
{
     ...
     */
    public TFieldCollapse(final AppMgr anAppMgr)
    {
        mAppMgr = anAppMgr;
    }
     ...
     */
    public TFieldCollapse(final AppMgr anAppMgr, String aCfgPropertyPrefix)
    {
        mAppMgr = anAppMgr;
        ...
        throws NSException
    {
        ...
        String propertyName = "field_collapse_file";
        ...
        if (schemaBag == null)
            throw new NSException("Connector schema file is undefined.");

        appLogger.trace(mAppMgr.LOGMSG_TRACE_DEPART);
    }

    private boolean isPropertyMultiValue(String aPropertyName)
    {
        String[] valueList = mCfgProperties.getStringArray(aPropertyName);
    ...
    private String[] getMultiValues(String aPropertyName)
    {
        String[] cfgValues;

        if (isPropertyMultiValue(aPropertyName))
            cfgValues = mCfgProperties.getStringArray(aPropertyName);
        else
        {
            cfgValues = new String[1];
            String cfgValue = mCfgProperties.getString(aPropertyName);
    ...
    private void addValue(DataField aField, String aValue)
    {
        if (StringUtils.isNotEmpty(aValue))
        {
            if (aField.isMultiValue())
                aField.addValue(aValue);
            else
            {
                String curValue = aField.getValue();
                if (StringUtils.isEmpty(curValue))
                    aField.setValue(aValue);
                else
                    aField.setValue(curValue + "," + aValue);
            }
        }
    }

    private HashMap<String,String> buildMappedFieldsFromSrcBag(DataBag aDstBag, DataBag aSrcBag)
    {
        String[] multiMapNames;
        DataField dstField, mapField;
        String schemaFieldName, colFieldName, mapFieldName;
        Logger appLogger = mAppMgr.getLogger(this, "buildMappedFieldsFromSrcBag");

        appLogger.trace(mAppMgr.LOGMSG_TRACE_ENTER);

        HashMap<String,String> hashMap = new HashMap<>();
        for (DataField schemaField : mBag.getFields())
        {
            schemaFieldName = schemaField.getName();
            colFieldName = mCfgProperties.getString(schemaFieldName);
            if (StringUtils.isNotEmpty(colFieldName))
            {
                mapFieldName = hashMap.get(schemaFieldName);
                if (StringUtils.isEmpty(mapFieldName))
                {
                    dstField = new DataField(schemaField);
                    dstField.setAssignedFlag(false);
                    if (isPropertyMultiValue(schemaFieldName))
                    {
                        multiMapNames = getMultiValues(schemaFieldName);
                        for (String mapName : multiMapNames)
                        {
                            hashMap.put(mapName, schemaFieldName);
                            mapField = aSrcBag.getFieldByName(mapName);
                            if (mapField != null)
                                addValue(dstField, mapField.getValue());
                        }
                    }
                    else
                    {
                        mapField = aSrcBag.getFieldByName(colFieldName);
                        if (mapField != null)
                            addValue(dstField, mapField.getValue());
                    }
                    if (dstField.isAssigned())
                    {
                        aDstBag.add(dstField);
                        hashMap.put(schemaFieldName, colFieldName);
                    }
                }
            }
        }
    ...
    private void transform(DataBag aDstBag, DataBag aSrcBag)
    {
        ...
        {
            HashMap<String,String> hashMap = buildMappedFieldsFromSrcBag(aDstBag, aSrcBag);
            for (DataField srcField : aSrcBag.getFields())
            {
                fieldName = srcField.getName();
                mapFieldName = hashMap.get(fieldName);
                if (StringUtils.isEmpty(mapFieldName))
                {
